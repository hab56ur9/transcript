...
- Skills route; sub-documents (like setup.md) carry one-time detail. Agents pair with a template file in templates/ — keep agent rules and template placeholders consistent.

Contracts to preserve when editing:

- The three-stage pipeline wording: audio → transcript → note, immutable inputs, lineage in frontmatter (engine, source_audio, template, source_transcript).
- Note agents must exclude generated notes and backfill transcripts when picking a default target.
