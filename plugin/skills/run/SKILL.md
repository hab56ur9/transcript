...
- USR1 requests recording. With Ask Before Recording on (the default, settings key confirm_recording), the app shows an approval alert dialog and recording begins only when the user confirms it; cancelling declines, and control signals queue behind an unanswered alert. The menu Record button starts immediately — clicking it is itself consent.
...
- On "전사 모드 시작": if the app is alive, send USR1. If not, launch the launcher with the record flag in the background. In both cases, also start the watch command in the background — it exits the moment recording ends by any path (window stop, menu, USR2, quit), which notifies the agent immediately. Tell the user to confirm the approval alert, confirm the state file reaches recording, then end the turn. The watch command tolerates the approval wait: it idles until recording begins (up to five minutes) before it starts watching for the end.
