...
- On "전사 모드 시작": if the app is alive, send USR1. If not, launch the launcher with the record flag in the background. Then also start the watch command in the background — it exits the moment recording ends by any path (window stop, menu, USR2, quit), which notifies the agent immediately. Confirm the red icon, then end the turn.
