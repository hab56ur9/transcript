...
Local realtime meeting transcriber for macOS. It lives in the menu bar, captures the microphone and system audio, labels every utterance by speaker, and streams text into a transcript file as people speak. Summaries are delegated to this plugin's agents: meeting-note for meetings, lecture-note for lectures, dev-doc for design discussions.
...
The app stays resident once launched. Recording is controlled by launcher subcommands that send signals under the hood (USR1 record, USR2 stop and save, TERM quit after save), so app lifetime and recording state stay independent.

- The start subcommand is idempotent: it launches the app into recording when nothing is running, and just begins recording when the app is already resident. The bare record flag behaves the same way.
- With Ask Before Recording on (the default, settings key confirm_recording), the app shows an approval alert dialog and recording begins only when the user confirms it; cancelling declines, and control requests queue behind an unanswered alert. The menu Record button starts immediately — clicking it is itself consent.
- The stop subcommand stops and saves; the app stays resident.
- The quit subcommand quits the app, saving first if a recording is active.
...
- On "전사 모드 시작": run the start subcommand, and also start the watch command in the background — it exits the moment recording ends by any path (window stop, menu, stop subcommand, quit), which notifies the agent immediately. Tell the user to confirm the approval alert, confirm the state file reaches recording, then end the turn. The watch command tolerates the approval wait: it idles until recording begins (up to five minutes) before it starts watching for the end.
...
- Starting a recording is the default whenever the AI launches the app. Launch bare (no subcommand) only when the user explicitly asks for idle standby.
- On "전사 모드 종료": run the stop subcommand — the running watch task will notify on completion and trigger the follow-up above. Without a watcher, wait on the state file directly. The meeting note is generated by the meeting-note subagent of this plugin (on hosts without subagents, follow the meeting-note document inline), fed the transcript path plus any memos the user shared during the meeting, and its TL;DR and saved location are relayed back to the user.
- Run the quit subcommand only on an explicit quit request.
- Menu bar actions coexist with the subcommands — always verify real state before acting.
...
- Alive + recording: normal. To finish, run the stop subcommand, wait for idle, then delegate to the meeting-note agent.
- Alive + paused: the user paused from the transcript window or menu; audio and transcription are both suspended and the live path stays. Resume happens in the UI; the stop subcommand still stops and saves.
- Alive + idle: resident standby. The last saved path is the latest result; the start subcommand records again.
...
- Hallucinated text during silence: common filler phrases (감사합니다, thank you for watching, …) are dropped by the built-in hallucination filter; for new phrases extend its list, or raise the silence threshold.
...
Models/HallucinationFilter.swift  phrases blocklist (exact match after normalization)
...
```bash
Models/    RecordingSession, ChunkSplitter, HallucinationFilter, Utterance, ServiceProtocols
